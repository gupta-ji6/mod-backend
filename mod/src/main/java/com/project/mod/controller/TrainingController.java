...
//		List<Skill> skills = training.getSkill();
//		System.out.println("Skills................"+skills);
		/*
		 * //training.getSkill().clear(); if(skills != null && skills.size() > 0) {
		 * for(int i=0; i<skills.size(); i++) { Skill skill =
		 * skillService.findById(skills.get(i).getId());
    ...
    public Training getTrainingsByIdAndUserId(@PathVariable(value = "id") Integer id, @PathVariable(value = "userId") Integer userId) {
        return trainingService.findByIdAndUser(id, userId);
    }
    ...
    public List<Training> findByStartDateBetween(@PathVariable(value="dateStart") String dateStart, @PathVariable(value="dateEnd") String dateEnd) {
    	...
    	//@PathVariable(value="dateStart") @DateTiemFormat(pattern = "YYYY-MM-dd") Date dateStart
